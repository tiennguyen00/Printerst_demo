...
export default {
  getFileById: async (fileId) => {
    return File.find({ _id: fileId }).then(
      async (file) => {
        if (file) {
  ...
  deleteFileById: async (fileId) => {
    return File.findOne({ _id: fileId }).then(
      async (file) => {
        if (!file)
  ...
  getAllCommentById: async (fileId) => {
    return Comment.find({ postID: fileId }).then(
      async (file) => {
        if (file.length === 0)
          return Promise.reject(
            new ServiceError(400, "Not comment is founded!")
          );
        return Promise.resolve(file);
      },
      async (error) => {
